...
public class Configuration {
  ...
  // mapper 接口注册器, 内部封装生产 mapper 接口代理逻辑
  protected final MapperRegistry mapperRegistry = new MapperRegistry(this);
  protected final InterceptorChain interceptorChain = new InterceptorChain();

  // typeHandler 主要用于在 PreparedStatement 设置参数时, java 类型转 数据库类型(jdbcType),
  // 在 ResultSet 获取值后 数据库类型(jdbcType)转 java 类型
  protected final TypeHandlerRegistry typeHandlerRegistry = new TypeHandlerRegistry(this);

  // 别名注册器 TypeAlias, 见器构造方法
  protected final TypeAliasRegistry typeAliasRegistry = new TypeAliasRegistry();
  protected final LanguageDriverRegistry languageRegistry = new LanguageDriverRegistry();

  // 解析 mapper.xml/注解 文件, 根据sql定义生成 MappedStatement
  // 其中 key 为 namespace + id (namespace.id)
  ...
  // 加载过的 mapper.xml 放入 loadedResources
